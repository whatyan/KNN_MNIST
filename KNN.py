...
def training_data():
    train_label = []
    trainingFileList = listdir('trainingDigits') #得到所有的训练文本
    m = len(trainingFileList)
    train_data = zeros((m, 1024))
    # 获取训练集的标签
    for i in range(m):
        file_name = trainingFileList[i] #所有训练集文件名
        # 得到训练集索引
        train_label.append(classnumCut(file_name))
        train_data[i, :] = img2vector('trainingDigits/%s' % file_name)
...
def main():
    ...
    train_label, train_data = training_data()
